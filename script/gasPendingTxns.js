...
// 10.12.22
// eth.getLogs para ver en la mempool
...
pendingTxns()
async function pendingTxns(){
    var txnsReceipt=[];
    let sender= false;
    latestBlock = await Web3Client.eth.getBlock('pending', true);
       ...
       if (element['gas'] > 100000){
        sender= true;
         ...
         const object ={
            ...
            url: new URL('https://etherscan.io/tx/'+element['hash'])
         }
    ...
    if (sender) {
        try{
            ...
            fileName = path.join(__dirname, 'pendingTnxs.json')
